...
func MockWhileExecute(variableStore *models.VariableStore, w statements.While) (bool, int, error) {
	...
	for {
		...
		// }
		if result {
			if success, err := statements.ExecuteStatements(w.Block, variableStore, nil, nil, nil); err != nil {
...
func TestExecuteWhileFalse(t *testing.T) {
	...
	result, err := whileStatement.Execute(nil, referencedValueStore, nil, nil)
...
func TestExecuteWhileWithFalseCondition(t *testing.T) {
	...
	referencedValueStore := models.NewReferencedValueStore()
	result, err := whileStatement.Execute(&variableStore, referencedValueStore, nil, nil)
